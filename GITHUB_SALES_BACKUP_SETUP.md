...
workflow reads the complete Sales Tracker dataset at 11:00 PM Asia/Kolkata,
compresses and encrypts it with AES-256-GCM, then uploads a dated asset to a
dedicated full-backup release in a separate private GitHub repository.
...
8. Confirm that the private backup repository contains a release named
   `Sales Tracker full backup` with an `.etbackup` asset.

## How the daily backup is organised

The 11:00 PM Asia/Kolkata workflow creates one complete snapshot containing
all available Sales Tracker months, whether open or closed, together with the
salesperson records, salesperson-name registry, daily sales, edit history and
month status. Each daily run adds a new dated encrypted `.etbackup` asset to
the `Sales Tracker full backup` release.

The **Back up now** button creates the same complete snapshot. The month being
displayed when the owner clicks the button does not limit the backup.

Older monthly releases such as `Sales backup 2026-09` and
`Sales backup 2026-08` came from the previous backup format. They remain valid
encrypted historical backups, but new runs use only the full-backup release.
...
Each new full-backup asset contains every month that existed when it was made.
Deleting a month in the application or Firestore does not alter older encrypted
snapshots. To remove that historical data from GitHub, manually delete every
full-backup asset that predates the deletion, or delete the entire
`Sales Tracker full backup` release. This is never triggered by the application.
...
salespeople, the salesperson-name registry, daily sales, timestamps, edit
history and month status for every backed-up month.
